...
class LucidPath_SalesRep_Block_Adminhtml_Order_Grid_Renderer_PaymentStatus extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract {

  public function render(Varien_Object $row) {
    if ($row->getData('admin_id') == 0) {
      return "";
    }

    // show only current user if not admin
    if (Mage::getSingleton('admin/session')->isAllowed('system/config') ||
      ...
      if ($admin->getId() == $row->getData('admin_id')) {
        $value =  $row->getData($this->getColumn()->getIndex());
        return ucfirst($value);
      }
    }
