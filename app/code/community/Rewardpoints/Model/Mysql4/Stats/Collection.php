...
//        $lastname  = $customersCollection->getAttribute('lastname');
    ...
    public function addUsedpointsbydate($store_id, $customer_id, $admin_gift = false){
	$statuses = Mage::getStoreConfig('rewardpoints/default/valid_statuses', $store_id);
        ...
        if ($admin_gift){
            $cols['points_spent'] = 'main_table.points_spent as nb_credit_spent';
        }
        ...
        if ($admin_gift){
            ...
            $this->getSelect()->where(" main_table.order_id = ?", Rewardpoints_Model_Stats::TYPE_POINTS_ADMIN);
        } else {
            $this->getSelect()->from($this->getTable('sales/order').' as orders', $cols_order);
        
            //"main_table.id = main_table_admin.room_type_id", 
            if (version_compare(Mage::getVersion(), '1.4.0', '>=')){
                $this->getSelect()->where(" main_table.customer_id = $customer_id 
        ...
        if (!$admin_gift){
            ...
            //$this->getSelect()->group('date_start');
    ...
    public function joinValidPointsOrder($customer_id, $store_id, $order_states, $spent = false, $remove_end = false, $remove_start = false)
    {
        $statuses = Mage::getStoreConfig('rewardpoints/default/valid_statuses', Mage::app()->getStore()->getId());
        $status_field = Mage::getStoreConfig('rewardpoints/default/status_used', Mage::app()->getStore()->getId());
