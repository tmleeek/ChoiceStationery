...
class Rewardpoints_Model_Stats extends Mage_Core_Model_Abstract
{
    ...
    public function getPointsWaitingValidation($customer_id, $store_id){
        ...
        if ($this->points_waiting != null){
            ...
            $collection = $this->getCollection();
            $collection->joinValidPointsOrder($customer_id, $store_id, array("new"), false, true, true);
    ...
    protected function getPointsReceivedReajustment($customer_id, $store_id) {
	    ...
	    $points = $this
            ...
            $points_admin_gift = $this
                        ->getResourceCollection()
                        ->addUsedpointsbydate($store_id, $customer_id, true);
            ...
            $acc_fix_points = 0;
            $last_date = null;
            if (($points->getSize() || $points_admin_gift->getSize()) && sizeof($arr_points_collection)){
                $left_over = 0;
                ...
                if ($points->getSize()){
                    foreach ($points as $current_point){
                        ...
                        $nb_credit_spent = $current_point->getData('nb_credit_spent');
                        
                        $points_array[] = array('date_used_stamp' => $date_used_stamp, 'nb_credit_spent' => $nb_credit_spent);
                    }
                }
                if ($points_admin_gift->getSize()){
                    foreach ($points_admin_gift as $current_point){
                        $date_used = $current_point->getData('date_order');
                        $date_used = ($date_used) ? $date_used : $current_point->getData('date_start');
                ...
                if (sizeof($points_array)){
                    foreach ($points_array as $current_point){
                        $date_used_stamp = $current_point['date_used_stamp'];
                        $nb_credit_spent = $point_left = $current_point['nb_credit_spent'];
                        $checked = false;
                        //$extra = 0;
                        ...
                        foreach($arr_points_collection as $key => $point_collection_value){
                            ...
                            //if ($date_from_stamp <= $date_used_stamp && $date_end_stamp >= $date_used_stamp && $today_stamp > $date_end_stamp && !$checked && $arr_points_collection[$key]['points_calculated'] > 0){
                                if ($point_left > 0){
                                    $calc = $arr_points_collection[$key]['points_calculated'] - $nb_credit_spent+$extra;
            ...
            //if (sizeof($arr_points_collection) && $acc_fix_points){
                foreach($arr_points_collection as $key => $point_collection_value){
                    ...
                    //if (($today_stamp > $date_end_stamp || $today_stamp < $date_from_stamp) && $date_end != ""){
    ...
    public function getPointsReceived($customer_id, $store_id){
        ...
        //FIX J2T - exp date
        //joinValidPointsOrder($customer_id, $store_id, $order_states, $spent = false, $remove_end = false, $remove_start = false)
        $collection->joinValidPointsOrder($customer_id, $store_id, $order_states, false, true, true);
