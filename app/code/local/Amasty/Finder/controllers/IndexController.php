...
class Amasty_Finder_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function searchAction()
    {
        ...
        if ($this->getRequest()->getParam('reset')){
            ...
            if (Mage::getStoreConfig('amfinder/general/reset_home') == 'home'){
                $backUrl = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB);
            }  else {
                $resetUrl = Mage::helper('core')->urlDecode($this->getRequest()->getParam('reset_url'));
                $resetUrl = $this->_getModifiedBackUrl($finder, $resetUrl);
