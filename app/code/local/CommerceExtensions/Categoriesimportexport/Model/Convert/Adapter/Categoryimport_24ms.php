			...
			if(isset($importData['category_id']) && $importData['category_id'] !="") {
				...
				if($catupdate->getId() > 0) {
						...
						if(isset($importData['category_id']) && $importData['category_id'] !="") {
							...
							$oldchilderncount = $cat->getChildrenCount();
							...
							if($position != "" && $position > 0) {
								...
								$copy->setId($new_id)->setPath($newpath)->setPosition(1)->setLevel($oldlevelid)->setAttributeSetId($oldattributesetid)->setChildrenCount($oldchilderncount)->save(); //added postion default of 1
							}
