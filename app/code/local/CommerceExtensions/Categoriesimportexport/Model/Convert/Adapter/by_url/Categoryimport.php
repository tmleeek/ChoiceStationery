		...
		if($catupdate->getId() > 0) {
				...
				if(isset($importData['category_image'])) {
					if($importData['category_image'] != "") {
						...
						$file = $importData['category_image'];
						$path_parts = pathinfo($file);
						$targetFileName = $path_parts['basename'];
						$sourceFilePath = Mage::getBaseDir('media') . DS . 'import' . DS . $targetFileName;
						
						if(file_exists($sourceFilePath) && !is_dir($sourceFilePath))
						{
								$fullpath = Mage::getBaseDir('media') . DS . 'import' . DS . $targetFileName;
								try {
									#echo "URL: " . $file . "FINALNAME: " . $file;
									#$filewithspacesreplaced = str_replace(" ","%20", $file); //fix for urls with spaces in the Url
									$ch = curl_init ($file);
				...
				/* ABILITY TO IMPORT THUMB IMAGE FOR CATEGORIES START */
					if(isset($importData['category_thumb_image']) && $importData['category_thumb_image'] != "") {
						...
						$file = $importData['category_thumb_image'];
						$path_parts = pathinfo($file);
						$targetFileName = $path_parts['basename'];
						$sourceFilePath = Mage::getBaseDir('media') . DS . 'import' . DS . $targetFileName;
