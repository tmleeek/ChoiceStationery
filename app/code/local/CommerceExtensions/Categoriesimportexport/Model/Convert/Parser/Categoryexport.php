    ...
    public function unparse()
    {
										...
										$batchExport = $this->getBatchExportModel()
												...
												foreach(explode(',',$subsubsubcategories) as $subsubsubcategoriesid)
												{
													...
													/* START OF 6th LEVEL CATEGORY EXPORT */
														$subcategoriesmodel6 = Mage::getModel('catalog/category')->load($_sub_sub_sub_sub_category->getId());
														$subcategories6 = $subcategoriesmodel6->getChildren();
														foreach(explode(',',$subcategories6) as $subcategoriesid6)
														{
															if($subcategoriesid6 > 0) {
																$_sub_category6 = Mage::getModel('catalog/category')->load($subcategoriesid6);
																
																$categories_path = $cat->getName() . $category_delimiter . $_categorytop->getName() . $category_delimiter . $_sub_category->getName() . $category_delimiter . $_sub_sub_category->getName(). $category_delimiter . $_sub_sub_sub_category->getName(). $category_delimiter . $_sub_sub_sub_sub_category->getName(). $category_delimiter . $_sub_category6->getName();
																
																$row7 = $this->setCategoryRowData($rootId, $categories_path, $_sub_category6, $store);
																$batchExport = $this->getBatchExportModel()
																		->setId(null)
																		->setBatchId($this->getBatchModel()->getId())
																		->setBatchData($row7)
				...
				$subsubsubcategoriesmodel = Mage::getModel('catalog/category')->getCollection()
									...
									/* START OF 6th LEVEL CATEGORY EXPORT */
											...
											->setStore($store)//sets store ID
											->addAttributeToFilter(array(array('attribute'=>'parent_id', 'eq'=>$_sub_sub_sub_sub_category->getId())))
											->addAttributeToSelect('*')//or any other attributes you need
											->setOrder('position'); 
											...
											foreach($subsubsubsubsubcategoriesmodel as $subsubsubsubsubcategoriesid)
														{
															if($subsubsubsubsubcategoriesid['entity_id'] > 0) {
																$_sub_sub_sub_sub_sub_category = Mage::getModel('catalog/category')->load($subsubsubsubsubcategoriesid->getId());
																
																$categories_path = $_categorytop->getName() . $category_delimiter . $_sub_category->getName() . $category_delimiter . $_sub_sub_category->getName(). $category_delimiter . $_sub_sub_sub_category->getName(). $category_delimiter . $_sub_sub_sub_sub_category->getName(). $category_delimiter . $_sub_sub_sub_sub_sub_category->getName();
								...
								foreach($subsubcategoriesmodel as $subsubcategoriesid)
								{
									if($subsubcategoriesid['entity_id'] > 0) {
										...
										$batchExport = $this->getBatchExportModel()
											...
											$subcategoriesmodel4 = Mage::getModel('catalog/category')->getCollection()
														->setStore($store)//sets store ID
														->addAttributeToFilter(array(array('attribute'=>'parent_id', 'eq'=>$_sub_sub_category->getId())))//first level from the tree
														->addAttributeToSelect('*')//or any other attributes you need
														->setOrder('position'); 
														
											foreach($subcategoriesmodel4 as $subcategoriesid4)
											{
												if($subcategoriesid4['entity_id'] > 0) {
													$_sub_category4 = Mage::getModel('catalog/category')->load($subcategoriesid4['entity_id']);
													
													$categories_path = $_categorytop->getName() . $category_delimiter . $_category->getName() . $category_delimiter . $_sub_category->getName(). $category_delimiter . $_sub_sub_category->getName(). $category_delimiter . $_sub_category4->getName();
													
													$row5 = $this->setCategoryRowData($rootId, $categories_path, $_sub_category4, $store);
											...
											$subcategoriesmodel5 = Mage::getModel('catalog/category')->getCollection()
															->setStore($store)//sets store ID
															->addAttributeToFilter(array(array('attribute'=>'parent_id', 'eq'=>$_sub_category4->getId())))//first level from the tree
															->addAttributeToSelect('*')//or any other attributes you need
															->setOrder('position'); 
															
															foreach($subcategoriesmodel5 as $subcategoriesid5)
															{
																if($subcategoriesid5['entity_id'] > 0) {
																	$_sub_category5 = Mage::getModel('catalog/category')->load($subcategoriesid5['entity_id']);
																	
																	$categories_path = $_categorytop->getName() . $category_delimiter . $_category->getName() . $category_delimiter . $_sub_category->getName(). $category_delimiter . $_sub_sub_category->getName(). $category_delimiter . $_sub_category4->getName(). $category_delimiter . $_sub_category5->getName();
																	
																	$row6 = $this->setCategoryRowData($rootId, $categories_path, $_sub_category5, $store);
											...
											/* START OF 6th LEVEL CATEGORY EXPORT */
															...
															$subcategoriesmodel6 = Mage::getModel('catalog/category')->getCollection()
															->setStore($store)//sets store ID
															->addAttributeToFilter(array(array('attribute'=>'parent_id', 'eq'=>$_sub_category5->getId())))//first level from the tree
															->addAttributeToSelect('*')//or any other attributes you need
															->setOrder('position'); 
																		
																		
															foreach($subcategoriesmodel6 as $subcategoriesid6)
															{
																if($subcategoriesid6['entity_id'] > 0) {
																	$_sub_category6 = Mage::getModel('catalog/category')->load($subcategoriesid6['entity_id']);
																	
																	$categories_path = $_categorytop->getName() . $category_delimiter . $_category->getName() . $category_delimiter . $_sub_category->getName(). $category_delimiter . $_sub_sub_category->getName(). $category_delimiter . $_sub_category4->getName(). $category_delimiter . $_sub_category5->getName(). $category_delimiter . $_sub_category6->getName();
																	$row7 = $this->setCategoryRowData($rootId, $categories_path, $_sub_category6, $store);
																	$batchExport = $this->getBatchExportModel()
																			->setId(null)
																			->setBatchId($this->getBatchModel()->getId())
															...
															/* START OF 7th LEVEL CATEGORY EXPORT */
															$subcategoriesmodel7 = Mage::getModel('catalog/category')->getCollection()
															->setStore($store)//sets store ID
															->addAttributeToFilter(array(array('attribute'=>'parent_id', 'eq'=>$_sub_category6->getId())))//first level from the tree
															->addAttributeToSelect('*')//or any other attributes you need
															->setOrder('position'); 
																		
																		
															foreach($subcategoriesmodel7 as $subcategoriesid7)
															{
																if($subcategoriesid7['entity_id'] > 0) {
																	$_sub_category7 = Mage::getModel('catalog/category')->load($subcategoriesid7['entity_id']);
																	
																	$categories_path = $_categorytop->getName() . $category_delimiter . $_category->getName() . $category_delimiter . $_sub_category->getName(). $category_delimiter . $_sub_sub_category->getName(). $category_delimiter . $_sub_category4->getName(). $category_delimiter . $_sub_category5->getName(). $category_delimiter . $_sub_category6->getName(). $category_delimiter . $_sub_category7->getName();
																	$row8 = $this->setCategoryRowData($rootId, $categories_path, $_sub_category7, $store);
																	$batchExport = $this->getBatchExportModel()
																			->setId(null)
																			->setBatchId($this->getBatchModel()->getId())
																			->setBatchData($row8)
