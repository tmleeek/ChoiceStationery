    ...
    public function unparse()
    {
					...
					if($this->getVar('rootids')!="") {
				    	...
				    	foreach ($categories as $_categorytop) {
							  ...
							  #echo "CATID: " . $_categorytop->getId();
								...
								//START CUSTOM CODE CATEGORY PRODUCT COUNT EXPORT
								$category_products_export = "";
								$resource = Mage::getSingleton('core/resource');
								$prefix = Mage::getConfig()->getNode('global/resources/db/table_prefix'); 
								$read = $resource->getConnection('core_read');
								
							    $select_qry = "SELECT count(product_id) as productCount FROM `".$prefix."catalog_category_product` WHERE category_id ='".$_categorytop->getId()."'";
								...
								{ 
									#$product = Mage::getModel('catalog/product')->load($catproductdata['product_id']);
							   		$category_products_export = $catproductdata['productCount'];
								}
								$row['category_products_count'] = $category_products_export;
								//END CUSTOM CODE CATEGORY PRODUCT COUNT EXPORT
							    ...
							    $select_qry = "SELECT count(product_id) as productCount FROM `".$prefix."catalog_category_product` WHERE category_id ='".$cat->getId()."'";
								$catrows = $read->fetchAll($select_qry);
								foreach($catrows as $catproductdata)
								{ 
									#$product = Mage::getModel('catalog/product')->load($catproductdata['product_id']);
					...
					foreach ($subcats as $_category)
					{
						...
						#{
							...
							$select_qry = "SELECT count(product_id) as productCount FROM `".$prefix."catalog_category_product` WHERE category_id ='".$_category->getId()."'";
							$catrows = $read->fetchAll($select_qry);
							foreach($catrows as $catproductdata)
							{ 
								#$product = Mage::getModel('catalog/product')->load($catproductdata['product_id']);
							...
							$row2['category_products_count'] = $category_products_export;
							//END CUSTOM CODE CATEGORY PRODUCT COUNT EXPORT
						...
						foreach ($subcategories as $_sub_category)
						{
							...
							if($subcategoriesid > 0) {
								...
								$select_qry = "SELECT count(product_id) as productCount FROM `".$prefix."catalog_category_product` WHERE category_id ='".$_sub_category->getId()."'";
								$catrows = $read->fetchAll($select_qry);
								foreach($catrows as $catproductdata)
								{ 
									#$product = Mage::getModel('catalog/product')->load($catproductdata['product_id']);
								...
								$row3['category_products_count'] = $category_products_export;
								//END CUSTOM CODE CATEGORY PRODUCT COUNT EXPORT
							  ...
							  #echo "CAT ID: " . $_category->getId();
								...
								foreach(explode(',',$subsubcategories) as $subsubcategoriesid)
								{
									if($subsubcategoriesid > 0) {
										...
										$select_qry = "SELECT count(product_id) as productCount FROM `".$prefix."catalog_category_product` WHERE category_id ='".$_sub_sub_category->getId()."'";
										$catrows = $read->fetchAll($select_qry);
										foreach($catrows as $catproductdata)
										{ 
											#$product = Mage::getModel('catalog/product')->load($catproductdata['product_id']);
										...
										$row4['category_products_count'] = $category_products_export;
										//END CUSTOM CODE CATEGORY PRODUCT COUNT EXPORT
										//START CUSTOM CODE CATEGORY PRODUCT EXPORT
										if($this->getVar('export_products_for_categories') == "true") {
											$category_products_export = "";
										...
										$batchExport = $this->getBatchExportModel()
												...
												foreach(explode(',',$subsubsubcategories) as $subsubsubcategoriesid)
												{
													if($subsubsubcategoriesid > 0) {
														...
														$select_qry = "SELECT count(product_id) as productCount FROM `".$prefix."catalog_category_product` WHERE category_id ='".$_sub_sub_sub_category->getId()."'";
														$catrows = $read->fetchAll($select_qry);
														foreach($catrows as $catproductdata)
														{ 
															#$product = Mage::getModel('catalog/product')->load($catproductdata['product_id']);
														...
														$row5['category_products_count'] = $category_products_export;
														//END CUSTOM CODE CATEGORY PRODUCT COUNT EXPORT
														//START CUSTOM CODE CATEGORY PRODUCT EXPORT
														if($this->getVar('export_products_for_categories') == "true") {
															$category_products_export = "";
														...
														$select_qry = "SELECT count(product_id) as productCount FROM `".$prefix."catalog_category_product` WHERE category_id ='".$_sub_sub_sub_sub_category->getId()."'";
														$catrows = $read->fetchAll($select_qry);
														foreach($catrows as $catproductdata)
														{ 
															#$product = Mage::getModel('catalog/product')->load($catproductdata['product_id']);
														...
														$row6['category_products_count'] = $category_products_export;
														//END CUSTOM CODE CATEGORY PRODUCT COUNT EXPORT
														//START CUSTOM CODE CATEGORY PRODUCT EXPORT
														if($this->getVar('export_products_for_categories') == "true") {
															$category_products_export = "";
														...
														$select_qry = "SELECT count(product_id) as productCount FROM `".$prefix."catalog_category_product` WHERE category_id ='".$_sub_sub_sub_sub_sub_category->getId()."'";
														$catrows = $read->fetchAll($select_qry);
														foreach($catrows as $catproductdata)
														{ 
															#$product = Mage::getModel('catalog/product')->load($catproductdata['product_id']);
														...
														$row7['category_products_count'] = $category_products_export;
														//END CUSTOM CODE CATEGORY PRODUCT COUNT EXPORT								
														//START CUSTOM CODE CATEGORY PRODUCT EXPORT
														if($this->getVar('export_products_for_categories') == "true") {
															$category_products_export = "";
