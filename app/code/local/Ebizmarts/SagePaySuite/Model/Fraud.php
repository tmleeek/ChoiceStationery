...
{
    public function updateThirdMan(Varien_Simplexml_Element $trn, $order = null)
    {
    ...
    public function getTransactionDetail($vendorTxCode)
    {
        ...
        $xmlCommand="<command>getTransactionDetail</command>";
        $xmlCommand .= "<vendor>{$this->_getCdata('vendor')}</vendor>";
        $xmlCommand .= "<user>{$this->_getCdata('api_username')}</user>";
        $xmlCommand .= "<vendortxcode>{$vendorTxCode}</vendortxcode>";

        $xml = "<vspaccess>";
        $xml .= $xmlCommand;
        $xml .= "<signature>".md5($xmlCommand.'<password>'.$pwd.'</password>')."</signature>";
        $xml .= "</vspaccess>";
