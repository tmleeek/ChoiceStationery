...
class Ebizmarts_SagePaySuite_PaypalexpressController extends Mage_Core_Controller_Front_Action
{
    ...
    private function _initCheckout()
    {
        ...
        $this->_checkout = Mage::getSingleton(
            'sagepaysuite/paypal_checkout', array(
            'quote' => $quote,
            )
        );
    }
...
//            if($redirectJavascript){
//                //redirect
//                $this->getResponse()->setBody("<script>window.location.href = '" . Mage::getUrl('checkout/onepage/success', array('_secure' => true)) . "';</script>");
//            }else{
        ...
        //Redirect to cart
//        if($redirectJavascript){
//            $this->getResponse()->setBody("<script>window.location.href = '" . Mage::getUrl('checkout/cart', array('_secure' => true)) . "';</script>");
//        }else {
//            $this->_redirect('checkout/cart', array('_secure' => true));
//        }
        return;
    ...
    public function callbackAction()
    {
        ...
        $this->_getSession()->setData("paypal_post", $this->getRequest()->getPost());

        //show processing message
        $this->getResponse()->setBody($this->getLayout()->createBlock('sagepaysuite/checkout_paypalcallback')->toHtml());
