...
class FME_Percentagepricing_Model_Price extends Mage_Catalog_Model_Product_Type_Price
{
    ...
    public function getPrice($product)
    {
        ...
        //get value of cost
        $finalPrice = $product->getData('cost');
        //get value of disable rule
         $ruleEnable = $product->getData('fme_rule_enable');
            ...
            if($collection->getData()!=NULL)
            {
                ...
                foreach($collection as $_collection)
                {
                    ...
                    if((in_array("0",explode(",",$_collection->getWebsiteIds())))|| in_array($storeId,explode(",",$_collection->getWebsiteIds())))
                    {
                        ...
                        {
                            /*print_r($_rules);
                            print_r($product->getData(trim($_rules['attribute'])));
                            print_r($_rules['operator']); exit();*/
                            ...
                            {
                                $result="0";
                                if(array_intersect($product->getCategoryIds(), explode(",",$_rules['value'])))
                                $result="1";
                            ...
                            else
                            {
                                ...
                                if($_rules['operator']=="==")
                                {
                                    if(in_array($product->getData(trim($_rules['attribute'])), explode(",",$_rules['value']),false))
                                    $result="1";
                                }
                                if($_rules['operator']=="!=")
                                {
                                ...
                                if($_rules['operator']==">=")
                                {
                                    if($product->getData(trim($_rules['attribute']))>= $_rules['value'])
                                    $result="1";
                                }
                                if($_rules['operator']=="<=")
                                {
        ...
        //$this->_applySpecialPrice($product, $finalPrice);
