...
class Idev_OneStepCheckout_AjaxController extends Mage_Core_Controller_Front_Action
{
    ...
    public function save_billingAction()
    {
        ...
        if(!Mage::helper('customer')->isLoggedIn()){
            // ajaxcontroller save_billingAction 1
            if ($helper->isYesSwitch_CollectTotal_AjaxcontrollerPhp_SaveBillingAction1()) {
    ...
    public function set_methods_separateAction()
    {
        ...
        // ajaxcontroller_php_setmethodsseparateaction_setflag_1
        if ($helper->isYesSwitch_CollectTotal_AjaxcontrollerPhp_SetMethodsSeparateAction_SetFlag_1()) {
            // This is the modified version,
            // This needs to be used if user selects shipping method,
            // but summary does not get refreshed correctly
            $this->_getOnepage()->getQuote()->setTotalsCollectedFlag(false)->collectTotals();
        }
         else {
             // this is how it was in original package
