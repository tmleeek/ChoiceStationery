...
class Magestore_Pdfinvoiceplus_Block_Adminhtml_Pdfinvoiceplus_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    {
		$this->getLayout()->getBlock('head')->addJs('pdfinvoiceplus/variables.js');
		...
		$this->getLayout()->getBlock('head')->addJs ('pdfinvoiceplus/window.js')
				->addItem('js_css','pdfinvoiceplus/magento.css');
    ...
    {
        if (Mage::registry('pdfinvoiceplus_data')
            && Mage::registry('pdfinvoiceplus_data')->getId()
        ) {
            return Mage::helper('pdfinvoiceplus')->__("Edit Template '%s'",
                                                $this->htmlEscape(Mage::registry('pdfinvoiceplus_data')->getTemplateName())
            );
        }
