...
class Magestore_Pdfinvoiceplus_Block_Adminhtml_Pdfinvoiceplus_Edit_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
{
    ...
    protected function _beforeToHtml()
    {
        $block =  $this->getLayout()
                                ->createBlock('pdfinvoiceplus/adminhtml_pdfinvoiceplus_edit_tab_systemtemplate')
                                ->toHtml();
        $companyinfo = $this
            ->getLayout()
        ...
        $this->addTab('system_template_section', array(
            'label'     => Mage::helper('pdfinvoiceplus')->__('Template'),
            'title'     => Mage::helper('pdfinvoiceplus')->__('Template'),
            'content'   => $block,
        ));
