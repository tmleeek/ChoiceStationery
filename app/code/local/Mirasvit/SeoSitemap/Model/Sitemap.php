...
class Mirasvit_SeoSitemap_Model_Sitemap extends Mirasvit_SeoSitemap_Model_Sitemap_Abstract
{
    ...
    protected $bigImages;
    protected $showbigImagesInfo = false; //true - show ignored big images (GD2 issue)
    ...
    public function getImageUrl($file, $item, $storeId)
    {
        ...
        if ($config->getIsEnableImageFriendlyUrls($storeId)) {
            ...
            try {
                ...
                if ($imageUrl === false) { //detect big images product IDs
                    $this->bigImages[] = $item->getId();
                }
    ...
    public function generateXml()
    {
        ...
        if ($this->showbigImagesInfo) {
            Mage::getSingleton('adminhtml/session')->addSuccess(
                Mage::helper('sitemap')->__('Too big images product IDs: %s', implode(',', $this->bigImages))
            );
        }
