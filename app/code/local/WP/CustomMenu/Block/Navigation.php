...
class WP_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    {
		$change_from	=array("stationery.html","printer-supplies.html","computer-supplies.html","machines.html","paper.html","furniture.html");
		$change_to		=array("office-supplies","inkjets-and-toners","computer-supplies","office-machines","paper-supplies","office-furniture");
        if (!$category->getIsActive()) return '';
        $html = array();
        $id = $category->getId();
        // --- Static Block ---
        ...
        if ($level == 0 && $drawPopup) {
            $html[] = '<a href="javascript:void(0);" rel="'.str_replace($change_from,$change_to,$this->getCategoryUrl($category)).'">';
        } else {
        ...
        $html[] = '<span class="icon '.strtolower(str_replace('&nbsp;', '', $name)).'"></span>' . $name ;
        $html[] = '</a>';
        $html[] = '</div>';
        $html[] = '</div>';
        // --- Add Popup block (hidden) ---
