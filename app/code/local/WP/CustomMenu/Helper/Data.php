...
class WP_CustomMenu_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getMobileMenuContent()
    {
        ...
        $homeLink           = '';
		//$page_url = Mage::helper('cms/page')->getPageUrl(9);
		//$page_text = $this->__('Printer Supplies');
        if ($_showHomeLink) {
            $homeLink = <<<HTML
...
HTML;
    ...
    public function getMenuContent()
    {
		...
		$source = Mage::getModel('cms/block')->load('wp_custom_menu_5');
		$processor = $helper->getPageTemplateProcessor();
		$html = $processor->filter($source->getContent());
		$printer_block2 = $html;
        //$printer_block2 = $this->getLayout()->createBlock('cms/block')->setBlockId('wp_custom_menu_5')->toHtml();
        if ($_showHomeLink) {
            $homeLink = <<<HTML
<div class="menu">
