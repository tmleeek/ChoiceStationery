...
class Webtex_CustomerGroupsPrice_Model_Observer
{
    ...
    public function layeredPrice($observer)
    {
        ...
        if(($customer = Mage::getModel('customer/session')->getCustomer()) && $customer->getGroupId()){
            ...
            $col = "IFNULL(
                        IF(
                            POSITION('%' IN cgp_sprices.price),
                            IF(
                                SUBSTRING(cgp_sprices.price, 1, 1) IN ('+', '-'),
                                IF(price_index.final_price IS NULL,price_index.min_price * (100 + cgp_sprices.price) / 100 ,price_index.final_price * (100 + cgp_sprices.price) / 100),
                                IF(price_index.final_price IS NULL,price_index.min_price * cgp_sprices.price / 100,price_index.final_price * cgp_sprices.price / 100)
                            ),
                            IF(
                                SUBSTRING(cgp_sprices.price, 1, 1) IN ('+', '-'),
                                IF(price_index.final_price IS NULL,price_index.min_price + cgp_sprices.price,price_index.final_price + cgp_sprices.price),
                                cgp_sprices.price
                            )
                        )
                    , IFNULL(
                        IF(
                            POSITION('%' IN cgp_prices.price),
                            IF(
                                SUBSTRING(cgp_prices.price, 1, 1) IN ('+', '-'),
                                IF(price_index.final_price IS NULL,price_index.min_price * (100 + cgp_prices.price) / 100 ,price_index.final_price * (100 + cgp_prices.price) / 100),
                                IF(price_index.final_price IS NULL,price_index.min_price * cgp_prices.price / 100,price_index.final_price * cgp_prices.price / 100)
                            ),
                            IF(
                                SUBSTRING(cgp_prices.price, 1, 1) IN ('+', '-'),
                                IF(price_index.final_price IS NULL,price_index.min_price + cgp_prices.price,price_index.final_price + cgp_prices.price),
                                cgp_prices.price
                            )
                        )
                    ,
    ...
    public function sortByPrice($observer)
    {
        ...
        if(!$isPriceOrder || !Mage::helper('customer')->isLoggedIn() || !Mage::helper('customergroupsprice')->isGroupActive($customer->getGroupId())){
            return $this;
        }
        ...
        if(!in_array('cgp_prices', array_keys($from))){
            ...
            $collection->getSelect()
                ...
                ->order('CONVERT(cgp_min_price, DECIMAL(12,4)) '.strtoupper($orderDir));
        }
    }
	...
	public function frontCollectionLoadAfter($observer)
	{
	  if(!Mage::helper('customergroupsprice')->isGroupActive(Mage::getSingleton('customer/session')->getCustomer()->getGroupId())) {
	      ...
	      $webtexHelper = Mage::helper('customergroupsprice');
	      $prices = Mage::getModel('customergroupsprice/prices');
	      ...
	      foreach($observer->getCollection() as $_item) {
	          ...
	          $specialPrice     = $specialPrices->getProductPrice($_item);
                  $specialPrice = min($price, $specialPrice);
	          ...
	          if($webtexHelper->isUsePriceRules()){
	...
	public function processFrontFinalPrice($observer)
	{
	  if(!Mage::helper('customergroupsprice')->isGroupActive(Mage::getSingleton('customer/session')->getCustomer()->getGroupId())) {
	    ...
	    if($product->getTypeId() != 'bundle') {
	          ...
	          $specialPrice     = $specialPrices->getProductPrice($product);
                  $specialPrice = min($basePrice, $specialPrice);
                  $specialPriceFrom = $product->getSpecialFromDate();
                  $specialPriceTo   = $product->getSpecialToDate();
	          if($webtexHelper->isUsePriceRules()){
    ...
    public function processQuoteItem($observer)
	{
        ...
        foreach($items as $key=>$item){
	        ...
	        $webtexHelper = Mage::helper('customergroupsprice');
	          ...
	          if($webtexHelper->isUsePriceRules()){
                      $rulePrice  = Mage::getModel('catalogrule/rule')->calcProductPriceRule($item, $price);
                  } else {
                      $rulePrice  = null ; 
                  }
                      $finalPrice = $product->getPriceModel()->calculatePrice(
	...
	public function processBackFinalPrice($observer)
	{
	  ...
	  if($rule_data) {
	   ...
	   if($data['customer_group_id'] && Mage::helper('customergroupsprice')->isGroupActive($data['customer_group_id'])) {
	    ...
	    if($product->getTypeId() != 'bundle') {
	          $webtexHelper = Mage::helper('customergroupsprice');
	          $price     = $prices->getGroupProductPrice($product,$data['customer_group_id'],$data['website_id']);
	          $sPrice    = $specialPrices->getGroupProductPrice($product,$data['customer_group_id'],$data['website_id']);
	          if($webtexHelper->isUsePriceRules()){
                      $rulePrice  = Mage::getModel('catalogrule/rule')->calcProductPriceRule($product, $price);
                  } else {
                      $rulePrice  = null ; 
