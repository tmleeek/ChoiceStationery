...
class Webtex_CustomerGroupsPrice_Model_Specialprices extends Mage_Core_Model_Abstract {
    ...
    public function getProductPrice($product) {
        ...
        if(!$productOriginalPrice){
            $productOriginalprice = $product->getGroupPrice();
        }
	    ...
	    if(isset($row['price']) && isset($row['price_type'])){
            ...
            if($productPriceString ){ //&& (float)$productPrice != 0){
	        if($pos = strpos($productPriceString,'%')){
	            if(in_array(substr($productPriceString, 0, 1), array('+', '-'))) {
	                $productPrice = $productPrice + $productOriginalPrice/100 * (float)$productPriceString ;
	            } else {
	                $productPrice = $productPrice/100 * (float)$productPrice ;
	            }
	        } elseif (in_array(substr($productPriceString, 0, 1), array('+', '-'))){
	            $productPrice = $productPrice + (float)$productPriceString ;
	        } else {
	    ...
	    if(isset($row['price']) && isset($row['price_type'])){
            ...
            if($productPriceString){ // && (float)$productPrice != 0){
                if($pos = strpos($productPriceString,'%')){
	                ...
	                $productPrice = $productPrice/100 * (float)$productPriceString ;
	            }
	        } elseif (in_array(substr($productPriceString, 0, 1), array('+', '-'))){
	            $productPrice = $productPrice + (float)$productPriceString ;
	        } else {
	            $productPrice =  (float)$productPriceString;
	        }
            }
