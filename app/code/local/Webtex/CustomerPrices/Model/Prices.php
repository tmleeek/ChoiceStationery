...
class Webtex_CustomerPrices_Model_Prices extends Mage_Core_Model_Abstract {
    ...
    public function getProductPrice($product, $qty = 1)
    {
        ...
        $websiteId = Mage::helper('customerprices')->getPriceWebsiteId();
        ...
        $query = $connection->select()->from($tablePrefix . 'customerprices_prices')->where('product_id = 0 AND customer_id = ' . $customerId . ' AND qty = 0 and store_id = '.$websiteId);
	...
	if(isset($row['discount'])){
        ...
        $query = $connection->select()->from($tablePrefix . 'customerprices_prices')->where('product_id = ' . $product->getId() . ' AND customer_id = ' . $customerId . ' AND qty <= '. $qty . ' and store_id = '.$websiteId);
        $row = $connection->fetchRow($query);
...
/*
	...
	if(isset($row['discount']) && $isRealSpecialPrice){
	    if($pos = strpos($row['discount'],'%')){
	        if(in_array(substr($row['discount'], 0, 1), array('+', '-'))) {
	            $productPrice = $productPrice + $realPrice/100 * (float)$row['discount'] ;
	        } else {
...
*/
    ...
    public function loadByCustomer($productId, $customerId, $qty = 1, $websiteId = 0)
    {
        if(is_null($qty)) {
            $qty = 1;
        }
        $this->setData($this->getResource()->loadByCustomer($productId, $customerId, $qty, $websiteId));
        return $this;
    }

    public function loadCustomerPriceByCustomer($customer_id,$product_id){
        if($customer_id!="" && $product_id!=""){
            $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
            $tablePrefix = (string) Mage::getConfig()->getTablePrefix();

            $query = $connection->select()->from($tablePrefix . 'customerprices_prices')->where('product_id = ' . $product_id. ' AND customer_id = ' . $customer_id)." limit 1";
            $row = $connection->fetchRow($query);
