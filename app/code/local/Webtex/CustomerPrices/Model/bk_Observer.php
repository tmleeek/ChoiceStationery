...
class Webtex_CustomerPrices_Model_Observer
{
  ...
  public function productLoadAfter($observer)
  {
      ...
      if($_session->getCustomerId() && $_session->isLoggedIn()) {
        ...
        if(!empty($customerprice) && $customerprice['qty']>1){
          $tierPrices[] = array(
            ...
            'website_price'  => $customerprice['price']
           );
      ...
      /*if($_SERVER['REMOTE_ADDR']=='43.243.39.90'){
        echo $product->getPrice()." ".$prices->getProductPrice($product)." test";
        exit;
      }*/
  ...
  public function processFrontFinalPrice($observer)
  {
      ...
      $prices = Mage::getModel('customerprices/prices');
      /*if($_SERVER['REMOTE_ADDR']=='43.243.39.90'){
        echo $product->getPrice()." ".$prices->getProductPrice($product)." test123";
        exit;
      }*/
  ...
  public function processBackFinalPrice($observer)
  {
    ...
    public function productAddToCartBefore($observer){
      //if($_SERVER["REMOTE_ADDR"]=="81.133.157.252"){
		$isLoggedIn = Mage::getSingleton('customer/session')->isLoggedIn();
		
		if(!$isLoggedIn){
			$my_file=Mage::getBaseDir()."/var/systemaddtocart.txt";
			...
			foreach($items as $item){
				...
				exit; */
				$data='';
				if($item->getPrice()<=0 || $item->getPrice()<=0.0000 || $item->getPrice()==''){
					//if($item->getId()!=""){
						try{
							$product_id = $item->getProductId();
							$product=Mage::getModel('catalog/product')->load($product_id);
							...
							$item->setPrice($price);
							$item->save();
							$data = "item price updated with ".$price." :::";
						}
						catch(Exception $ex){
							$data = "item price not updated:::";
						}
					//}

					$data .= "item-id:".$item->getId()."=> product-id:".$item->getProduct()->getId()."=> prodcut-price:".$item->getPrice()."\n";
					fwrite($handle, $data);
				}
			}
		}
