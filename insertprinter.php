...
$product = Mage::getModel('catalog/product');
 ...
 if($productId)
 {
      ...
      $sql = "select * from catalog_product_entity_text
       where store_id=0 and attribute_id=717 and entity_id=$productId"; 
        ...
        $value_id=$rows[0]['value_id'];
         ...
         if(!$rows){
         $query = "INSERT INTO `catalog_product_entity_text`(`entity_type_id`, `attribute_id`, `store_id`, `entity_id`, `value`) VALUES (4,717,0,$productId,'$model')"; 
         $writeConnection->query($query);
	 }else
	 {
		 
		 if($value_id){
			 $query ="UPDATE `catalog_product_entity_text` SET `value`='$model' WHERE `value_id`=$value_id and `attribute_id`=717 and `store_id`=0";
			 $writeConnection->query($query);
		 }
	 }
          echo "<br>";
