				...
				foreach($rowchield as $rowchieldObj){
					...
					if($thirdcategoryid > 0){
						...
						foreach($rowprodsku as $rowprodskuObj){
							...
							$_product->save();*/
							if($_product){
								$marrayvalue = $maincatid."-".$_product->getId();
								$sarrayvalue = $secondcatid."-".$_product->getId();
								$tarrayvalue = $thirdcatid."-".$_product->getId();
								if (!in_array($marrayvalue, $assignProArry)){
									Mage::getSingleton('catalog/category_api')->assignProduct($maincatid,$_product->getId());
									array_push($assignProArry,$marrayvalue);
								}
								if (!in_array($sarrayvalue, $assignProArry)){
									Mage::getSingleton('catalog/category_api')->assignProduct($secondcatid,$_product->getId());
									array_push($assignProArry,$sarrayvalue);
								}
								if (!in_array($tarrayvalue, $assignProArry)){
									Mage::getSingleton('catalog/category_api')->assignProduct($thirdcatid,$_product->getId());
									array_push($assignProArry,$tarrayvalue);
								}
							}
							echo "\n--".$productskus." - updated <br />";
