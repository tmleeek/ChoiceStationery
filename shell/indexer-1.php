...
class Mage_Shell_Compiler extends Mage_Shell_Abstract
{
        ...
        if ($this->getArg('info')) {
            ...
            try {
                ...
                foreach ($processes as $process) {
					...
					/* print_r(get_class($process));
					print_r(get_class_methods($process));
					exit; */
					if($this->getArg('reindexall') && $process->getIndexerCode()=="catalogsearch_fulltext")
					{
						continue;
					} 
                    /* @var $process Mage_Index_Model_Process */
                    try {
						//	$this->sendMailAction("reindex-start - ". $process->getIndexer()->getName());
                        $process->reindexEverything();
                        Mage::dispatchEvent($process->getIndexerCode() . '_shell_reindex_after');
                        $msg = $process->getIndexer()->getName() . " index was rebuilt successfully\n";
                    } catch (Mage_Core_Exception $e) {
                        $msg = $e->getMessage() . "\n";
                    } catch (Exception $e) {
                        $msg = $process->getIndexer()->getName() . " index process unknown error:\n";
                        $msg .= $e . "\n";
                    }
					echo $msg;
				//	$this->sendMailAction($msg);
                }
                Mage::dispatchEvent('shell_reindex_finalize_process');
            } catch (Exception $e) {
                Mage::dispatchEvent('shell_reindex_finalize_process');
                echo $e->getMessage() . "\n";
				//$this->sendMailAction($e->getMessage() );
            }
	...
	public function sendMailAction($msg=''){
		if($msg)
		{
				$html="Reindex Status : ".$msg;
				
					
			     Mage::log($msg, null, 'reindex.log');
				$mail = Mage::getModel('core/email');
				$mail->setToName('Choice Stationary');
				$mail->setToEmail('bhavinj@rocktechnolabs.com');
				$mail->setBody($html);
				$mail->setSubject('Reindex Mail');
				$mail->setFromEmail('admin@choicestationary.com');
				$mail->setFromName("Choice Stationary Reindex");
				$mail->setType('html');// You can use Html or text as Mail format

				try {
					$mail->send();
					mail("bhavinj@rocktechnolabs.com","Reindex Mail",$html);
					echo "success";
				}
				catch (Exception $e) {
					echo $e->getMessage();
